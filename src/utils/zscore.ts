...
export function generateSignal(bars: KlineBar[]): SignalResult {
  ...
  if (!z20) {
    return {
      direction: 'none',
      zScore20: 0, zScore120: 0, latestClose,
      ma5: 0, ma20: 0, ma120: 0, std20: 0, avgTurnover: 0,
      suggestion: '',
      blockedByNewLow: false, blockedByMA5: false, trendOk: false,
      diagnostic: `数据不足（需20根K线，当前${bars.length}根）`,
    };
  }

  // 5-day moving average
  const recent5 = bars.slice(-5);
  const ma5 = recent5.reduce((s, b) => s + b.close, 0) / recent5.length;
  ...
  const blockedByMA5 = bars.length >= 5 && latestClose <= ma5;
  ...
  if (direction === 'none') {
    ...
    if (blockedByMA5) blockers.push('跌破5日线');
    if (!trendOk) blockers.push(`Z120=${fmtZ(z120s)} ≥ 0 大势不配合`);

    if (!blockedByNewLow && !blockedByMA5 && trendOk) {
