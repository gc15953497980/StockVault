...
import { GOLD_MINING_STOCKS, goldUsdToCny } from '../utils/goldApi';
    ...
    <div className={styles.container}>
      ...
      <div className={styles.tableBox}>
        ...
        <table className={styles.table}>
          ...
          <tbody>
            {GOLD_MINING_STOCKS.map(s => {
              const info = miningStockPrices[s.code];
              return (
                <tr key={s.code}>
                  <td>{s.code.replace(/^(sh|sz)/, '')}</td>
                  <td>{s.name}</td>
                  <td>{info ? info.price.toFixed(2) : '-'}</td>
                  <td className={info && info.changePercent >= 0 ? styles.up : styles.down}>
                    {info ? formatPercent(info.changePercent) : '-'}
                  </td>
                </tr>
              );
            })}
